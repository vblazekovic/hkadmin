...
if __name__ == "__main__":
    ...
    with st.form("comp_form"):
        ...
        # Treneri: multiselect iz baze + dodatni unos (auto-upis u bazu)
        ...
        for cname in coach_all:
            if cname and cname not in coach_choices:
                try:
    ...
    comp_rows = conn.execute("SELECT id, name, date_from, kind FROM competitions ORDER BY date_from DESC").fetchall()
    ...
    if choice:
        ...
        with st.form("edit_comp_form"):
            ...
            # Treneri: multiselect + dodatni (auto-upis)
            coach_rows = conn.execute("SELECT full_name FROM coaches ORDER BY full_name").fetchall()
            coach_choices = [r[0] for r in coach_rows] if coach_rows else []
                    ...
                    with b1:
                        if st.button("Spremi izmjene", key=f"btn_save_res_{rid}"):
                            conn.execute("""UPDATE competition_results
                                            ...
                                            WHERE id=?""", (member_id_e, weight_e, style_e, placing_e, result_text_e, rid))
                            conn.commit()
