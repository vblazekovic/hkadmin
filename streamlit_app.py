...
# ---------------- Natjecanja i rezultati ----------------
    ...
    comps = pd.read_sql_query("SELECT id, name || ' ' || IFNULL(date_from,'') AS naziv FROM competitions ORDER BY date_from DESC", conn)
    ...
    # ----- Uredi / obriši rezultate + EXPORT -----
    st.markdown("---")
    st.subheader("Uredi / obriši rezultate")

    comps_edit = pd.read_sql_query(
    ...
    if comps_edit.empty:
        ...
        # Export gumbi (CSV/Excel) za odabrano natjecanje
        if not rdf.empty:
            colx = st.columns(2)
            with colx[0]:
                csv_data = rdf.to_csv(index=False).encode("utf-8")
                st.download_button("Preuzmi rezultate (CSV)", data=csv_data, file_name=f"rezultati_natjecanje_{comp_id_e}.csv", mime="text/csv", key="dl_csv_res")
            with colx[1]:
                from io import BytesIO
                buf = BytesIO()
                with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
                    rdf.to_excel(writer, index=False, sheet_name="Rezultati")
                st.download_button("Preuzmi rezultate (Excel)", data=buf.getvalue(), file_name=f"rezultati_natjecanje_{comp_id_e}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_xlsx_res")
        ...
        if rdf.empty:
            ...
            for _, rowr in rdf.iterrows():
                rid = int(rowr["id"])
                naslov = f"#{rid} – {rowr['sportas']} • {rowr['weight_category'] or ''} • {rowr['placing'] or ''}"
                with st.expander(naslov):
                    c1, c2 = st.columns(2)
                    with c1:
                        try:
                            default_idx = imena.index(rowr["sportas"])
                        except ValueError:
                            default_idx = 0
                        member_name_e = st.selectbox("Sportaš", imena, index=default_idx, key=f"res_member_{rid}")
                    ...
                    with b1:
                        if st.button("Spremi izmjene", key=f"btn_save_res_{rid}"):
                            conn.execute("""UPDATE competition_results
                                            SET member_id=?, weight_category=?, style=?, placing=?, result_text=?
                                            WHERE id=?""",
                                         (member_id_e, weight_e, style_e, placing_e, result_text_e, rid))
                            conn.commit()
